...
const resultSections = (items = []) => {
  const { shorts, videos } = splitVideos(items);
  return `
    ${shorts.length ? `<section class="section-block"><div class="section-head"><h2>ショート</h2></div><section class="short-grid">${shorts.map((item) => videoCard(item, 'short')).join('')}</section></section>` : ''}
    ${videos.length ? `<section class="section-block"><div class="section-head"><h2>動画</h2></div><section class="video-grid">${videos.map((item) => videoCard(item)).join('')}</section></section>` : ''}
  `;
};
...
export const homePage = (trending, region = 'US') => pageShell(`
  ...
  ${resultSections(trending || [])}
`, 'AuroraTube', '', 'home');

export const trendingPage = (trending, region = 'US') => pageShell(`
  <section class="page-head">
    <div>
...
export const searchPage = (query, filters, items = []) => pageShell(`
  <section class="page-head">
    <div>
  ...
  ${resultSections(items)}
  ${channelSections(items)}
...
export const watchPage = (payload = {}) => {
  ...
  return pageShell(`
    <article class="viewer">
      ${playerMarkup({ videoId, poster, short: false })}
...
export const shortsPage = (payload = {}) => {
  ...
  return pageShell(`
    <article class="viewer viewer-short">
      ${playerMarkup({ videoId, poster, short: true })}
