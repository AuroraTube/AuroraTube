...
const buildDownloadUrl = (videoId) => `/api/watch/${encodeURIComponent(videoId)}/download`;

export const playerMarkup = ({ videoId = '', poster = '', short = false } = {}) => {
  const safeVideoId = escapeAttr(videoId);
  const streamUrl = safeVideoId ? buildStreamUrl(videoId) : '';
  const downloadUrl = safeVideoId ? buildDownloadUrl(videoId) : '';

  return `
    <section class="player-frame ${short ? 'player-frame-short' : ''}" data-player data-video-id="${safeVideoId}" data-stream-url="${escapeAttr(streamUrl)}" data-download-url="${escapeAttr(downloadUrl)}">
      <div class="player-stage">
        <video class="player-video" playsinline preload="metadata"${poster ? ` poster="${escapeAttr(poster)}"` : ''} src="${escapeAttr(streamUrl)}"></video>
      ...
      <div class="player-controls">
        <div class="player-row player-row-main">
          <button class="player-icon-button" type="button" data-player-play-toggle aria-label="再生/一時停止">再生</button>
...
const syncState = (root) => {
  ...
  const updatePlayState = () => {
    ...
    if (playCenter) playCenter.textContent = playing ? '❚❚' : '▶';
    if (playCenter) playCenter.hidden = playing;
  };
