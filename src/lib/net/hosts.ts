...
/** True when hostname is a known YouTube/Invidious image CDN (or listed instance). */
export function isAllowedImageHost(hostname: string, extraOrigins: readonly string[] = []): boolean {
  const host = hostname.toLowerCase()
  if (isBlockedHostname(host)) return false
  if (IMAGE_SUFFIXES.some((s) => host === s || host.endsWith(`.${s}`))) return true
