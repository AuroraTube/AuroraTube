...
import { toProxiedHlsUrl, toProxiedMediaUrl } from '@/lib/mediaProxy'
 ...
 * signatures) when requested straight from the page origin. HLS sources are
 * *always* proxied (toProxiedHlsUrl), independent of the user's media-proxy
 * Settings toggle, since hls.js/native HLS cannot function without it.
 */
export function useVideoSource({ videoRef, url, isHls, resume }: Options): Result {
  ...
  useEffect(() => {
    ...
    if (isHls) {
      // HLS always goes through the media proxy, regardless of the
      // Settings toggle (which only governs progressive playback).
      const proxiedUrl = toProxiedHlsUrl(url)
      const preferNative =
