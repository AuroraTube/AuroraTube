...
type Options = {
  ...
  /** Bump to force the loading/error state to reset after a manual reload. */
  retryKey?: number
}
...
export function usePlayerUi({
  ...
  const mediaLoading = useMediaLoading({ videoRef, audioRef, usesSplitAudio, mediaKey, retryKey })
  const {
    controlsVisible,
    setControlsVisible,
    showControls,
